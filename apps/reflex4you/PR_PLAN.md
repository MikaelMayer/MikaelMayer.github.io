...
- [ ] Detect axis constraints via a dedicated AST analysis pass that records every context for each finger slot. Only mark a slot as axis-constrained if **all** of its usages resolve to the same axis primitive (`x`, `real` synonyms for X, or `y`, `imag` for Y); repeated occurrences are fine, but a second axis or any non-axis usage clears the constraint back to "free".
- [ ] Ensure the detection walks unambiguous, specific nodes (e.g., the `VarX`/`VarY` siblings produced after parsing `x`, `real`, etc.) so the gesture layer can trust the classification.
- [ ] During interaction, clamp that slot's movement to the relevant axis solely when the analysis reports a single-axis constraint.
...
- [ ] Add support for `floor` (new unary function) that maps directly to the existing complex helpers, with parser + GLSL generation updates plus tests.
...
- [ ] Update relevant tests to assert both AST structure and metadata retention (including the new `real`/`imag` and `floor` syntax).

## 6. Documentation & Cleanup
- [ ] Create `apps/reflex4you/README.md` (or expand the existing one) to document the formula language primitives, new parser keywords (`real`, `imag`, `floor`), and the interaction constants (`F`, `D`, `W` semantics plus gesture rules and axis constraints).
- [ ] Update the roadmap/developer docs with the new finger semantics and traversal helper.
